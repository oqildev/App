...
function toggleSidebar() {
    ...
    if (wasCollapsedAndPeeking) {
        // Pin peekProgress at 1 for the duration of the collapse animation so visualExpansion
        // stays at 1 the whole way (no shrink-then-grow flicker). Once collapseProgress = 0,
        // cp=0 makes visualExpansion=1 regardless of pp, so the reset is invisible.
        isPeeking = false;
        peekProgress.set(1);
