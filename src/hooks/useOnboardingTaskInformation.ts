...
function useOnboardingTaskInformation(taskName: IntroSelectedTask) {
    ...
    // True when a task report ID exists but its report hasn't hydrated into Onyx yet, so callers can
    // wait instead of treating it as "no task" (which would skip completion on a timing race).
    const isLoadingTaskReport = !!taskReportID && isLoadingOnyxValue(taskReportMetadata);
    return {
        taskReportID,
        taskReport,
        taskParentReport,
